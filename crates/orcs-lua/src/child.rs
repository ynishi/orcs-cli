...
/// Overrides file tools with capability-gated versions (child variant).
///
/// Uses [`ContextWrapper`] in `app_data` to check capabilities at call time.
///
/// Capability mapping:
...
fn register_capability_gated_tools(
    ...
    fn with_context<F, R>(lua: &Lua, f: F) -> mlua::Result<R>
    where
    ...
    tracing::debug!("child: overrode file tools with capability-gated versions");
    Ok(())
}
...
fn register_context_functions(
    ...
    register_capability_gated_tools(lua, &orcs_table, sandbox)?;
